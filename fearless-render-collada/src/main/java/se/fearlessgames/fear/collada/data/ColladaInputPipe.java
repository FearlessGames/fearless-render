...
public class ColladaInputPipe {
	...
	public ColladaInputPipe(final ColladaDOMUtil colladaDOMUtil, final Element input) {
		...
		if (accessor != null) {
			logger.info("Creating buffers for: " + _source.getAttributeValue("id"));
			...
			if (_paramCount > 1 && skippedParam) {
				logger.warn("Parameter skipping not yet supported when parsing sources. " + _source.getAttributeValue("id"));
			}
	...
	public void setupBuffer(final int numEntries, final Mesh mesh) {
		...
		switch (_type) {
			case POSITION:
				_buffer = createFloatBuffer(size);
				mesh.setVertexBuffer(_buffer);
				break;
			case NORMAL:
				_buffer = createFloatBuffer(size);
				mesh.setNormalBuffer(_buffer);
				break;
			case TEXCOORD:
				_buffer = createFloatBuffer(size);
				mesh.setTextureCoords(_texCoord, _buffer);
				break;
			default:
		}
	}
	...
	public void pushValues(final int memberIndex) {
		...
		if (memberIndex >= _sourceData.count) {
			logger.warn("Accessed invalid index " + memberIndex + " on source " + _source + ".  Count: " + _sourceData.count);
			return;
		}
